...
server <- function(input, output, session) {
  ...
  results <- eventReactive(input$runSimulation, {
    ...
    # C++ doesn't return NA for invalid use of AD with log relative risk
    if (measure == 1 && (method == 6 || method == 9 || method == 12))
      out = list(NA, NA, NA, NA, NA, NA)
    else
      out = simulationCpp(input$N, input$p0, input$p1, input$burnin, input$nsim, method, measure)
